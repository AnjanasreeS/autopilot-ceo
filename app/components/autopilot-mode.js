...
import { Badge } from "@/components/ui/badge"
import { Switch } from "@/components/ui/switch"
import { Zap, Users, CheckCircle, Clock, AlertCircle, Mail } from "lucide-react"
import { motion } from "framer-motion"

const dummyEmails = [
  {
    subject: "Urgent: Client X dissatisfaction",
    content: "Survey scores dropped by 15%. Immediate follow-up needed.",
    team: "Sales",
    assignee: "Sarah Johnson",
    priority: "high",
    status: "assigned",
  ...
  {
    subject: "APAC Revenue Growth",
    content: "Sales surged 34%. Marketing budget review recommended.",
    team: "Marketing",
    assignee: "Mike Chen",
    priority: "medium",
    status: "in-progress",
...
export default function AutopilotMode() {
  ...
  useEffect(() => {
    if (autopilotEnabled) {
      const newTasks = dummyEmails.map((email, i) => ({
        id: Date.now() + i,
        title: email.subject,
        ...
        status: email.status,
        aiReasoning: `Extracted insight from email: "${email.subject}"`,
        estimatedTime: "2 hours",
        deadline: "Today, 5:00 PM",
      }))
      setTasks(newTasks)
  ...
  const getStatusIcon = (status) => {
    return {
      ...
      assigned: <AlertCircle className="w-4 h-4 text-orange-500" />,
    }[status] || <Clock className="w-4 h-4 text-gray-500" />
  }
  ...
  return (
    ...
    >
      <Card className="bg-gradient-to-br from-sky-900 to-sky-700 border border-blue-300 shadow-xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-200 text-sm mt-1">AI auto-assigns tasks from analyzed email triggers</p>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-sm font-medium text-white">{autopilotEnabled ? "Active" : "Inactive"}</span>
              <Switch checked={autopilotEnabled} onCheckedChange={setAutopilotEnabled} />
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-slate-800 p-4 rounded-lg border border-blue-200 text-white">
              <div className="text-2xl font-bold">24</div>
              <div className="text-sm text-slate-300">Auto-Assigned</div>
            </div>
            <div className="bg-slate-800 p-4 rounded-lg border border-blue-200 text-white">
